...
app.post('/send-email', async (req, res) => {
  ...
  for (let { email, password } of emails) {
    ...
    const transporter = nodemailer.createTransport({
      service: 'gmail',
      auth: {
        user: email, // Email from the request
        pass: password, // Password from the request
      },
    });
    ...
    let mailOptions = {
      ...
      to: emails.map(item => item.email).join(','), // Join array of emails into a comma-separated string
